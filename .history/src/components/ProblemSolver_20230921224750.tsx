...
function ProblemSolver() {
  ...
  // Calculate the number of rows for the code editor based on available space
  useEffect(() => {
    const submitButtonHeight = 48; // Adjust as needed
    const selectMenuHeight = 32; // Adjust as needed
    const availableSpace = window.innerHeight - submitButtonHeight - selectMenuHeight;
    const estimatedRowHeight = 20; // Adjust as needed

    const calculatedRows = Math.floor(availableSpace / estimatedRowHeight);
    setCode(code.substring(0, calculatedRows));
  }, [window.innerHeight]);
